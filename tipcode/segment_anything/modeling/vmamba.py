...
if True:
    try:
        ...
        _HAS_SELECTIVE_SCAN_CUDA_CORE = True
    except Exception:
        selective_scan_cuda = None
        _HAS_SELECTIVE_SCAN_CUDA_CORE = False

    _WARNED_FVM_FALLBACK = False

    if _HAS_SELECTIVE_SCAN_CUDA_CORE:
        ...
        class SelectiveScan(torch.autograd.Function):
            ...
            def forward(ctx, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, nrows=1):
                raise ImportError(
                    "缺少依赖：selective_scan_cuda_core（CUDA 扩展）。"
                    "若要启用 FVM/Mamba 分支，请先安装/编译对应扩展；"
                    "否则将使用降级实现（仅用于可运行性/调试，结果会与论文不同）。"
                )

            @staticmethod
            def backward(ctx, dout, *args):
                raise ImportError("缺少依赖：selective_scan_cuda_core（CUDA 扩展）。")

    if "selective_scan_fn_v1" not in globals():
        def selective_scan_fn_v1(u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False):
            return SelectiveScan.apply(u, delta, A, B, C, D, delta_bias, delta_softplus, 1)

    if "selective_scan_ref_v1" not in globals():
...
class senet(nn.Module):
    def __init__(self,c=32,r=4):
        super().__init__()
        #self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.forward_ssm = VSSBlock
        self.fc = nn.Sequential(nn.Conv2d(c,c//r,1,1,0,bias=True),nn.SiLU(),nn.Conv2d(c//r,c,1,1,0,bias=True))
...
class SS2D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.norm_yty = nn.LayerNorm(d_model)
        ...
        # channle
        # x proj; dt proj =======================
        self.x_proj_forward = nn.Linear(1, (self.dt_rank + self.d_state * 2), bias=False)
        self.dt_proj_forward = self.dt_init(self.dt_rank, 1, dt_scale, dt_init, dt_min, dt_max, dt_init_floor, **factory_kwargs)

        # A, D =======================================
        self.A_log_forward = self.A_log_init(self.d_state, 1, merge=True)  # (K * D, N)
        self.D_forward = self.D_init(1, merge=True)  # (K * D)

        # x proj; dt proj =======================
        self.x_proj_backward = nn.Linear(1, (self.dt_rank + self.d_state * 2), bias=False)
        self.dt_proj_backward = self.dt_init(self.dt_rank, 1, dt_scale, dt_init, dt_min, dt_max,
                                             dt_init_floor, **factory_kwargs)

        # A, D =======================================
        self.A_log_backward = self.A_log_init(self.d_state, 1, merge=True)  # (K * D, N)
        self.D_backward = self.D_init(1, merge=True)  # (K * D)
        # out proj =======================================
    ...
    def forward_single_forward(self, x: torch.Tensor):
        selective_scan = selective_scan_fn_v1
        ...
        try:
            ...
            B, L, d = x.shape
        x = x.permute(0, 2, 1)
        x_dbl = self.x_proj_forward(rearrange(x, "b d l -> (b l) d"))  # (bl d)
        #print(x_dbl.size())
        dt, B, C = torch.split(x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=-1)
        dt = self.dt_proj_forward.weight @ dt.t()
        dt = rearrange(dt, "d (b l) -> b d l", l=L)
        A = -torch.exp(self.A_log_forward.float())  # (k * d, d_state)
        B = rearrange(B, "(b l) dstate -> b dstate l", l=L).contiguous()
        C = rearrange(C, "(b l) dstate -> b dstate l", l=L).contiguous()

        y = selective_scan(
            x, dt,
            A, B, C, self.D_forward.float(),
        ...
        y = self.out_norm(y)
        return y
    def forward_single_backward(self, x: torch.Tensor):
        selective_scan = selective_scan_fn_v1
        try:
            B,L,d=x.shape
        except:
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.d_conv > 1:
            ...
            z = z.permute(0,3,1,2)
            z = self.avgpool(z).squeeze().unsqueeze(-1)#b,c,1,1
            z_for = self.forward_single_forward(z)
            z_back = torch.flip(self.forward_single_backward(torch.flip(z,dims=[1])),dims=[1])
            z = z_for + z_back
            #z = z.permute(0,2,1)
            #z = self.out_norm(z)
            z = z.unsqueeze(1)
...
class _FallbackSS2D(nn.Module):
    """
    当 selective_scan_cuda_core（CUDA 扩展）不可用时，用一个轻量的 DWConv+PWConv
    近似替代 SS2D（仅用于可运行性/调试，结果会与论文不同）。
    """

    def __init__(self, d_model: int, dropout: float = 0.0, **kwargs):
        super().__init__()
        self.dw = nn.Conv2d(d_model, d_model, kernel_size=3, padding=1, groups=d_model, bias=False)
        self.pw = nn.Conv2d(d_model, d_model, kernel_size=1, bias=True)
        self.act = nn.GELU()
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if _HAS_SELECTIVE_SCAN_CUDA_CORE:
            ...
            global _WARNED_FVM_FALLBACK
            if not _WARNED_FVM_FALLBACK:
                warnings.warn(
                    "selective_scan_cuda_core 未检测到：FVM/Mamba 将自动降级为 depthwise-conv 近似实现。"
                    "该降级仅用于可运行性/调试，若需复现论文结果请安装 CUDA 扩展。",
                    RuntimeWarning,
                )
                _WARNED_FVM_FALLBACK = True
            self.op = _FallbackSS2D(d_model=hidden_dim, dropout=attn_drop_rate)
...
class Backbone_VSSM(VSSM):
    ...
    def load_pretrained(self, ckpt=None, key1="state_dict",key2="backbone"):
        if ckpt is None:
            return
        ...
        try:
            ...
            if key1 in _ckpt:
                target_dict = _ckpt[key1]
                if key2 in target_dict:
                    target_dict_ = target_dict[key2]
                #for key in target_dict.keys():
                #    print(key)
            print(f"Successfully load ckpt {ckpt}")
            incompatibleKeys = self.load_state_dict(target_dict_, strict=False)
            print('1')
