...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor, fre_adapter: torch.nn.Module):
        """
        前向传播
        Args:
            x: 输入图像 [B, C, H, W]
            fre_adapter: 频率适配器模块列表（FGSAttn_Adapter）
        Returns:
            x: 图像特征 [B, out_chans, H', W']
            attention_maps: 每层的频率注意力图列表，每个元素为 [B, H, W]
        """
        ...
        attention_maps = []  # 收集每层的频率注意力图
        for i, blk in enumerate(self.blocks):
            x = blk(x)
            # FGSAttn_Adapter 返回增强特征和注意力图
            x, attn_map = fre_adapter[i](x)
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        #,fre:torch.Tensor,fre_adapter:torch.nn.Module
        shortcut = x
        x = self.norm1(x)
        #fre = self.norm1(fre)
        ...
        x_1 = shortcut + x
        #print(x.size())
        x = self.mlp(self.norm2(x_1))
        #fre_next,fre_res = fre_adapter(fre,x)
        x = x+x_1
